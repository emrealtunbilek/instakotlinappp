...
class TakipNewsRecyclerAdapter(var context: Context, var takipcileriminTumBildirimleri: ArrayList<BildirimModel>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun getItemViewType(position: Int): Int {
        if (takipcileriminTumBildirimleri.get(position).bildirim_tur == Bildirimler.GONDERI_BEGENILDI) {

            return Bildirimler.GONDERI_BEGENILDI

        } else if (takipcileriminTumBildirimleri.get(position).bildirim_tur == Bildirimler.TAKIP_ETMEYE_BASLADI) {
            return Bildirimler.TAKIP_ETMEYE_BASLADI
        } else return 0
    }

    override fun getItemCount(): Int {
        return takipcileriminTumBildirimleri.size
    }

    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {

        var oankiBildirim = takipcileriminTumBildirimleri.get(position)
                        ...
                        FirebaseDatabase.getInstance().getReference().child("users").child(kimiTakipEtmisUserID).addListenerForSingleValueEvent(object : ValueEventListener {
                            ...
                            override fun onDataChange(p0: DataSnapshot?) {
                                ...
                                if(p0!!.getValue()!=null){
                                    ...
                                    bildirim.setText("takipEttigimUserName " + kimiTakipEtmisUserName+" kullanıcısını takip etmeye başladı")
